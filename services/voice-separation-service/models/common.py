...
These models are shared across services for common operations.
"""
...
class ErrorResponse(BaseModel):
    ...
    detail: str = Field(
        ...
        json_schema_extra={"example": "Invalid file type provided"},
    )
    error_code: Optional[str] = Field(
        default=None,
        description="Optional error code for programmatic handling",
        json_schema_extra={"example": "INVALID_FILE_TYPE"},
    )
...
class StatusResponse(BaseModel):
    ...
    message: str = Field(
        ...
        json_schema_extra={"example": "Voice Separation Service"},
    )
    timestamp: datetime = Field(
        default_factory=datetime.utcnow,
